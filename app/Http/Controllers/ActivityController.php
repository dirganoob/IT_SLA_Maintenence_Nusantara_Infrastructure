...
class ActivityController extends Controller
{
    ...
    public function toll(Request $request)
    {
        ...
        $selected_value2 = $request->kategori;
        ...
        // if ($request->has('search2') || $selected_value2) {
        //     $toll = Activity::with('user')
        //         ->where(function ($query) use ($request, $selected_value2) {
        //             $query->where('kategori_activity', 'Toll')
        //                 ->whereIn('status', ['pending']);
        //             if ($request->has('search2')) {
        //                 $query->where('kategori', 'like', '%' . $request->search2 . '%');
        //                 session()->put('selected_value', null);
        //             } else {
        //                 $query->where('kategori', '=', $selected_value2);
        //                 session()->put('selected_value', $selected_value2);
        //             }
        //         })
        //         ->latest()
        //         ->paginate($pagination);
        ...
        if (($request->has('search') && $request->input('search')) || ($selected_value)) {
            $searchTerm = $request->has('search') ? $request->input('search') : $selected_value;
        
            $toll = Activity::with('user')
                ->where(function ($query) use ($searchTerm) {
                    $query->where('kategori_activity', 'Toll')
                        ->whereIn('status', ['pending'])
                        ->where('lokasi', 'like', '%' . $searchTerm . '%');
                })
                ->latest()
                ->paginate($pagination);
        
            session()->put('selected_value', $searchTerm);
            session()->put('toll', $toll);
        } elseif (($request->has('search2') && $request->input('search2')) || ($selected_value2)) {
            $searchTerm2 = $request->has('search2') ? $request->input('search2') : $selected_value2;
        
            $toll = Activity::with('user')
                ->where(function ($query) use ($searchTerm2) {
                    $query->where('kategori_activity', 'Toll')
                        ->whereIn('status', ['pending'])
                        ->where('kategori', 'like', '%' . $searchTerm2 . '%');
                })
                ->latest()
                ->paginate($pagination);
        
            session()->put('selected_value', $searchTerm2);
            session()->put('toll', $toll);
        } 
        
        elseif (($request->has('start_date') && $request->input('start_date')) || ($request->has('end_date') && $request->input('end_date'))) {
            ...
            $toll = Activity::with('user')
                ->where(function ($query) use ($startDate, $endDate) {
                    $query->where('kategori_activity', 'Toll')
                        ->whereIn('status', ['pending'])
                        ->whereBetween('tanggal', [Carbon::parse($startDate), Carbon::parse($endDate)]);
                })
                ->latest()
                ->paginate($pagination);
        }
